...
{
	static uint8_t l_cnt = 0;        //统计低电平经过了几个100us
	static uint8_t h_cnt = 0;        //统计高电平经过了几个100us
	static uint8_t l_cnt_save = 0;   //记录发生上升沿之后上一次低电平经过了几个100us
	static uint8_t h_cnt_save = 0;   //记录发生下降沿之后上一次高电平经过了几个100us
	uint8_t falling_flag = 0; //发生下降沿的标志位，看波形发生一次下降沿就有一个位传输完成或者即将有一个位传输
	
	if (GPIO_ReadInputDataBit(GPIOA, GPIO_Pin_8) == Bit_RESET) {
		if (l_cnt == 0) { //低电平且第一次记录说明发生了一次下降沿
			h_cnt_save = h_cnt;
			h_cnt = 0;
			falling_flag = 1;
		}
		...
		if (h_cnt == 0) { //高电平且第一次记录说明发生了一次上升沿
			l_cnt_save = l_cnt;
			l_cnt = 0;
		}
		h_cnt++;
	...
	if (falling_flag == 1)
		ir_decode_bit(l_cnt_save, h_cnt_save);
}
