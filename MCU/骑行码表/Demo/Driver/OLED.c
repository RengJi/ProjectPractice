...
/* OLED屏地址*/
#define OLED_ADDRESS (0x3C<<1) /* 0x78 */
...
void OLED_WriteCommand(unsigned char Command)
{
	...
	cmd[0] = 0x00;
	cmd[1] = Command;
	soft_i2c_write(OLED_ADDRESS, cmd, sizeof(cmd));
}
...
void OLED_WriteData(unsigned char Data)
{
	...
	data[0] = 0x40;
	data[1] = Data;
    soft_i2c_write(OLED_ADDRESS, data, sizeof(data));
}

/**
  * @brief  OLED设置光标位置
  * @param  Page 以左上角为原点，向下方向的坐标，范围：0~7
  * @param  Column 以左上角为原点，向右方向的坐标，范围：0~127
  * @retval 无
  */
...
{      	
	...
	OLED_SetCursor(0, 0);		//设置光标位置在上半部分
	for (i = 0; i < 8; i++)
	{
		OLED_WriteData(H[i]);   //显示上半部分内容
	}
	
	OLED_SetCursor(1, 0);	    //设置光标位置在下半部分
	for (i = 8; i < 16; i++)
	{
		OLED_WriteData(H[i]);	//显示下半部分内容
	}
}
